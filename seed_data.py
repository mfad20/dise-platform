...
Usage en local : python seed_data.py (réinitialise entièrement la base).
populate() est aussi appelé automatiquement par l'application au démarrage
si la base est vide (voir app/__init__.py) — utile sur un hébergement à
disque éphémère (ex. Render free tier) où la base repart à zéro à chaque
déploiement.
"""
...
def populate():
    """Crée le jeu de données de démonstration. Suppose un contexte d'app actif
    et une base déjà créée (db.create_all()) — n'efface rien elle-même."""
    # -------------------------------------------------------------
...
def run():
    """Réinitialise entièrement la base puis la repeuple. Usage local uniquement
    (python seed_data.py) — jamais appelé automatiquement par l'application."""
